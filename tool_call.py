...
# --- Invoke 2: Follow-up on same thread (tests memory) ---
...
# --- Invoke 3: Via @traceable for LangSmith tracing ---
@traceable
def call_graph(query:str):
    result=graph.invoke({"messages":[{"role":"user","content":query}]},config=config)
    return result["messages"][-1].content

print("Turn 3:", call_graph("i want to buy 20 GOOG stocks using current price.Then 10 apple. what will be the total cost?"))
